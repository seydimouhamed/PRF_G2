...
$tabrjz=[];
    ...
    public  function getpromogroupeapprenant($id,$id1)
    {
         ...
         $groupe=$this->repoGroupe->findBy(['promotion'=>$id,"id"=>$id1], ['id' => 'DESC']);
 

         $tab['referentiels']=$promo->getReferentiel();
         $tab['formateurs']=$promo->getFormateurs();
         $tab_apprenant=[];
         foreach($groupe->getApprenants() as $apprenant)
         {
            $tab_apprenant[]=$apprenant->getFisrtName()." ".$apprenant->getLastName();
         }
         $groupe->setApprenant($tab_apprenant);
