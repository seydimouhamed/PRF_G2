...
class ReferentielController extends AbstractController
{
    ...
    public function getReferentielGroupeCompetence(Request $request,EntityManagerInterface $entityManager)
    {
        ...
        public function putReferentielGCId(EntityManagerInterface $entityManager,$id,Request $request)
        {
               ...
               //ajout/suppression les groupe compétence au référentiel
               $method="";
               foreach($data as $k  => $dt)
               {
                         $bool= $k=="add"?false:true;
                         $method= $k=="add"?"add":"remove";
                            foreach($dt as $id_gc)
                            {
                                if(($bool && in_array($id_gc,$tab_id_gc)) || (!$bool && !in_array($id_gc,$tab_id_gc)) )
                                {
                                    $method="ca entre";
                                    $groupComp= $entityManager->getRepository(GroupeCompetence::class)->find($id_gc);
                                    if($groupComp)
                                    {
                                        $referentiel->{$method."GrpCompetence"}($groupComp);
...
!in_array et in_array
               //$entityManager->flush();

             return $this->json($method,201);  
